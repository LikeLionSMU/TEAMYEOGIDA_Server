...
CREATE TABLE `productsearch` (
  ...
  `search_personpid` varchar(45) DEFAULT NULL,
  `search_productpid` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`idx`),
  KEY `fk_search_personpid_idx` (`search_personpid`),
  KEY `productsearch_ibfk_1` (`search_productpid`),
  CONSTRAINT `fk_search_personpid` FOREIGN KEY (`search_personpid`) REFERENCES `profile1` (`kakaopid`) ON DELETE CASCADE ON UPDATE CASCADE,
